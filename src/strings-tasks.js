...
function isPalindrome(str) {
  const lowerRegString = str.replace(/[^a-z0-9]/gi, '').toLowerCase();

  const reversedString = lowerRegString.split('').reverse().join('');

  return reversedString === lowerRegString;
}
...
function findLongestWord(sentence) {
  ...
  return splitString.reduce((longest, current) => {
...
function reverseWords(str) {
  ...
  const reversedWords = splitString.map((item) =>
...
function extractNameFromTemplate(value) {
  const nameReg = /Hello, (\w+) (\w+)!/;

  const matchString = value.match(nameReg);
...
function encodeToRot13(str) {
  function replaceChar(match) {
    const shift = match <= 'M' || (match >= 'a' && match <= 'm') ? 13 : -13;
...
function getCardId(value) {
  ...
  const cardMark = value.charAt(value.length - 1);
  const cardLetter = value.substring(0, value.length - 1);

  const markIndex = marks.indexOf(cardMark);
  const letterIndex = array.indexOf(cardLetter);

  return markIndex * array.length + letterIndex;
}
